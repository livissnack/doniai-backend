    ...
    <div class="field table-toobar is-grouped">
      ...
      <div class="handle-list">
        ...
        <b-dropdown aria-role="list" position="is-bottom-left">
          ...
          <b-dropdown-item aria-role="listitem">导出用户</b-dropdown-item>
      ...
      <b-tab-item label="用户列表">
        ...
        >
          <template slot-scope="props">
            ...
            <b-table-column field="username" label="用户名" sortable>{{ props.row.username }}</b-table-column>
            <b-table-column field="status_text" label="状态" sortable>
              <span class="tag is-success">{{ props.row.status_text }}</span>
            </b-table-column>
            <b-table-column field="nikename" label="昵称" sortable>{{ props.row.nikename }}</b-table-column>
            ...
            <b-table-column field="origin_text" label="组织" sortable>{{ props.row.origin_text }}</b-table-column>
            <b-table-column field="access_nums" label="访问次数" sortable>{{ props.row.access_nums }}</b-table-column>
            <b-table-column field="balance" label="账户余额" sortable>{{ props.row.balance }}</b-table-column>
            <b-table-column field="empiric_value" label="经验值" sortable>{{ props.row.empiric_value }}</b-table-column>
...
<script>
import { getUsers, destroyUser } from "@/services/hspx";
...
export default {
  ...
  methods: {
    async getUsers () {
      try {
        const { data } = await getUsers({ page: this.pagination.currentPage, perPage: this.pagination.perPage });
        this.data = data.data;
